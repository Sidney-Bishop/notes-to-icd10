...
class ArtifactConfig:
    ...
    def _create_directories(self) -> None:
        ...
        Iterates through nested path values in the config and ensures
        each directory exists relative to project_root.
        """
        paths_config = self.config.get("paths", {})
        for path_val in self._extract_values(paths_config):
            # Skip file paths (contain extensions), only create directories
            if not any(path_val.endswith(ext) for ext in ['.parquet', '.csv', '.json', '.yaml', '.py']):
...
#     """
#     Project-wide source of truth for the Surgical Era.
#     Now supports Polars/DuckDB Medallion Architecture.
#     """
#     _instance = None
#     # REMOVED: _duckdb_active_conn - We will no longer try to manage a single active connection
#     # within the config object itself. Instead, get_duckdb_conn will always return a new connection.
...
#                     self.config_path = current / "artifacts.yaml"
#                     break
#                 current = current.parent
#             else:
...
#         # Paths for the new architecture
#         self.db_path = self.resolve_path("data", "db") / "audit_trail.ddb"
#         self.log_path = self.resolve_path("outputs", "evaluations") / "processing_log.jsonl"
...
#     def _create_directories(self) -> None:
#         """Automates folder creation for Raw, Silver, Gold, and DB."""
#         for path_val in self._extract_values(self.config.get("paths", {})):
#             # Ensure path_val is treated as relative to project_root
#             (self.project_root / path_val).mkdir(parents=True, exist_ok=True)

#     def _extract_values(self, node: Dict[str, Any]) -> List[str]:
#         out = []
#         for v in node.values():
#             if isinstance(v, dict): out.extend(self._extract_values(v))
#             elif isinstance(v, str) and not any(v.endswith(ext) for ext in ['.py', '.json', '.yaml']):
#                 out.append(v)
#         return list(set(out))
...
#     # --- REVISED: DuckDB Connection Management ---
#     def get_duckdb_conn(self):
#         """
#         Returns a NEW DuckDB connection to the audit trail database.
#         Each call creates a separate connection. The caller is responsible for closing it.
#         """
#         # print(f"Creating new DuckDB connection to {self.db_path}") # Optional: for debugging
#         return duckdb.connect(str(self.db_path))

#     def close_duckdb_conn(self):
#         """
#         This method is now a NO-OP (no operation) placeholder.
#         DuckDB connections obtained via get_duckdb_conn() are new instances
#         and should be closed directly by the caller using `con.close()`.
#         This method is included for backward compatibility or if other parts
#         of the code expect it, but it performs no action related to DuckDB connections.
#         """
#         # print("Note: config.close_duckdb_conn() is a NO-OP. Connections should be closed directly by the caller.")
#         pass # This method now does nothing.

#     # --- UPDATED: Polars Registration [cite: 2026-02-18] ---
#     def register_dataframe(self, name: str, df: pl.DataFrame, layer: str = "silver", phase: str = "EDA"):
#         """Saves a Polars dataframe to the specified Medallion layer."""
#         output_dir = self.resolve_path("data", layer)
#         file_path = output_dir / f"{name}.parquet"
        
#         df.write_parquet(file_path) # Polars native write
        
#         self.log_event(
#             phase=phase,
#             action=f"register_{layer}_df",
#             details={"name": name, "path": str(file_path), "shape": df.shape}
#         )
#         print(f"✓ Registered {layer} artifact '{name}' ({df.shape[0]} rows)")
#         return file_path

#     def load_dataframe(self, name: str, layer: str = "silver") -> pl.DataFrame:
#         """Loads a Polars dataframe from the Medallion layer."""
#         path = self.resolve_path("data", layer) / f"{name}.parquet"
#         return pl.read_parquet(path)

#     def log_event(self, phase: str, action: str, details: Dict[str, Any]):
#         entry = {"timestamp": datetime.now().isoformat(), "phase": phase, "action": action, **details}
