...
itself, or — more insidiously — the code's human-readable *description*. A
model that learns to detect these instances rather than the underlying clinical
signal achieves artificially high accuracy on synthetic data but fails on real
notes. This risk proved central to the present study: our code-pattern
redaction removed the codes but not the descriptions, and the resulting
description-level leakage inflated the headline accuracy and even reversed the
apparent ranking of model backbones. Guarding against verbatim *code* leakage
is not sufficient; description-level leakage must be checked for explicitly.
